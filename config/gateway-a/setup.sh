...
# Script de configuración automática para gateway-a
# Este script configura WireGuard y las rutas necesarias

echo "Configurando gateway-a..."
...
iptables -t nat -I POSTROUTING 1 -s 10.0.0.0/24 -d 172.16.10.0/24 -j MASQUERADE

# Configurar rutas adicionales para Site-to-Site
echo "Configurando rutas para Site-to-Site..."
ip route add 172.16.20.0/24 dev wg0 2>/dev/null || echo "Ruta a oficina B ya existe"
...
echo "Probando conectividad con gateway-b..."
ping -c 3 10.0.0.2 >/dev/null 2>&1 && echo "Conectividad con gateway-b: OK" || echo "Conectividad con gateway-b: FAIL"

echo "Configuración de gateway-a completada exitosamente"
